...
// `Style`, `ViewStyle` and `TextStyle` accept array types of their values 
...
export function createViewStyle(shf: StylesheetFactory, 
                                rem: number): ViewStyleObj {
  const flexStyles   = createFlexStyles(shf),
        posStyles    = createPositionStyles(shf, rem),
        dimStyles    = createDimensionStyles(shf, rem),
        spaceStyles  = createSpacingStyles(shf, rem),
        borderStyles = createBorderStyles(shf, rem),
...
export function createTextStyles(shf: StylesheetFactory, 
                                 rem: number): TextStyleObj {
  const fontStyles = createFontStyles(shf, rem)
  return {...fontStyles}}
...
type FontStyleKey =  ("fs1"  | "fs2"  | "fs3"  | "fs4" | "fs5" | "fs6" | "fs7" | "fs8" |
                      "fsI"  | "fwN"  | "fwB"  |
