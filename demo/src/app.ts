...
const todo = h<{todo: Todo, onUpdate: TodoAction, onRemove: TodoAction}>(p => {
...
const newTodo = h<{onCreate: Function}>(p => {
  ...
  return c.vw(
    ...
    c.btn({style: ["ml2", "mnw3"], onPress: onSubmit}, "New"))})
           
const todoList = h<{todos: Todo[], onUpdate: Function, onRemove: Function}>(p => {
  return c.vw(
    p.todos
...
export const app = h(() => {
  ...
  return c.vw(
    ...
    c.vw(
      {style: ["w6"]},
      newTodo({onCreate: o => setTodos([o].concat(todos))}),
      todoList({todos, 
                onUpdate: o1 => setTodos(todos.map(o2 => o1.id === o2.id ? {...o1, ...o2} : o2)),
                onRemove: o1 => setTodos(todos.filter(o2 => o1.id !== o2.id))})
    ))})
